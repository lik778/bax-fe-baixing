  ...
  <div class="bg">
    <div class="white-bg">
      <main>
        ...
        >
          <el-form-item label="推广关键字" prop="keyword">
            <el-input
              :disabled="isEdit"
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        keyword: [
          { required: true, message: "请填写核心产品" },
          {
            validator: (rule, value, callback) => {
              if (!/^[\u4E00-\u9FA5A-Za-z0-9]{2,10}$/.test(value)) {
                return callback(new Error("非特殊字符且单个词长度2-10个字"));
              }
              callback();
            },
            trigger: "blur"
        ...
        areas: [
          ...
          },
          {
            validator: (rule, value, callback) => {
              if (value.length !== 2) {
                return callback(new Error("需要选择2个区域"));
              }
              callback();
            },
            trigger: "blur"
