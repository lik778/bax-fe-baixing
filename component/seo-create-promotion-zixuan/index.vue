<template>
  <div class="create-promotion">
    <section class="promotion-target">
      ...
      <p class="warning"
         v-if="showExpireWarning">站点6个月内将过期，请选择其他站点，或<router-link :to="{name: 'seo-charge'}">购买
        </router-link>新官网</p>
    </section>
    ...
    <section>
      ...
      <div>
        <el-tag class="kw-tag"
                v-for="(kw, index) in promotion.keywords"
                :key="index"
                closable
                @close="() => {promotion.keywords.splice(index, 1)}">{{kw.word}}</el-tag>

        <input class="keyword-input"
               v-model.trim="inputKeyword"
               @keyup.enter="onEnter" />
        <el-button size="small"
                   @click="onEnter">确定</el-button>
      </div>
    ...
    <section>
      ...
      <div>
        <p>您已选择<span class="warning">{{promotion.keywords.length}}</span>个自选词，将创建<span
                class="warning">{{promotion.keywords.length}}</span>个推广计划。</p>
        <p>根据您所选择的关键词个数，预扣款<span
                class="warning">{{promotion.keywords.length * 600}}</span>元，预计可推广<span
                class="warning">2个月</span>（60天），到期将自动续费，为保证您的站点持续展现，请保持首页宝排名资金充足。</p>
      </div>
