    ...
    <el-table :data="keywords">
      <el-table-column label="关键词" key="word">
        <template slot-scope="{ row }">
          ...
          <span class="update-word" v-if="row.isUpdate">(修改)</span>
          <span class="good-word" v-if="RECOMMAND_SOURCES.includes(row.recommandSource)">(好词)</span>
        </template>
      </el-table-column>
      <el-table-column label="电脑端排名" key="cpcRanking" v-if="showPropRanking" :formatter="(r) => fmtCpcRanking(r.cpcRanking || -1)" />
      <el-table-column label="手机端排名" key="mobileCpcRanking" :formatter="(r) => fmtCpcRanking(r.mobileCpcRanking || -1)" />
      <el-table-column key="status">
        <table-header-tip slot="header" label-html="关键词状态" :tip-html="keywordStatusTip" />
        <div slot-scope="{row}">
          <el-tooltip v-if="row.status === KEYWORD_CHIBI_REJECT" placement="top" effect="dark" :content="getRefuseReason(row)">
            <span>
      ...
      <el-table-column key="price" min-width="220">
        ...
        <div slot="header" slot-scope="col">
          <table-header-tip :label-html="maxPriceLabel" :tip-html="cpcTopPriceTip" />
          <el-popover placement="top" v-model="pricePopoverVisible">
            <div>
              ...
              <div class="actions">
                <el-button size="mini" @click="pricePopoverVisible = false">取消</el-button>
                <el-button type="primary" size="mini" @click="changeAllWordPrice">确定</el-button>
              </div>
            </div>
            <a href="javascript:;" slot="reference" class="price-action">批量改价</a>
           </el-popover>
        </div>
        <div slot-scope="{row}">
          <span class="price">
      ...
      <el-table-column key="matchType" min-width="220">
        <!-- eslint-disable-next-line -->
        <div slot="header" slot-scope="col">
          <table-header-tip :label-html="matchTypeLabel" :tip-html="matchTypeTip" />
          <el-popover placement="top" v-model="matchTypePopVisible">
            <div>
              ...
              <div class="actions">
                ...
                <el-button type="primary" size="mini" @click="changeMatchType">确定</el-button>
              </div>
            </div>
            <a href="javascript:;" slot="reference" class="price-action">批量修改</a>
          </el-popover>
        </div>
        <div slot-scope="{row}" class="match-type">
          <el-select :value="row.matchType"
                     ...
                     @change="v => changeColMatchType(row, v)">
      ...
      <el-table-column label="操作" key="deletable">
        <div slot-scope="{row}">
          <el-button type="danger" size="mini" @click="deleteWord(row)">删除</el-button>
        </div>
      </el-table-column>
    </el-table>
  </div>
