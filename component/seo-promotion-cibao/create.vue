  ...
  <div class="cibao-page-container">
    <section>
      ...
      <div class="section-inline">
        ...
        >
          <el-radio-button v-for="item in durations" :label="item" :key="item">
            {{ item }}天
          </el-radio-button>
        </el-radio-group>
      </div>
      <div class="section-inline">
        ...
        <el-radio-group size="medium" v-model="promotion.volume">
          <el-radio-button v-for="item in volumes" :label="item" :key="item">
            {{ item }}词
          </el-radio-button>
        </el-radio-group>
      </div>
    ...
    <section>
      ...
      <p class="warning" v-if="showExpireWarning">
        站点{{ promotion.duration }}天内将过期， 请选择其他站点，或<router-link
          :to="{ name: 'seo-charge' }"
          >购买</router-link
        >新官网
      </p>
      <p class="warning" v-if="showExistWebsite">
        该站点已创建首页宝加速词包计划，请更换
      </p>
    </section>
    <section>
    ...
    <section>
      ...
      <div>
        ...
        <p>
          预扣款：{{ charge }}元，可推广{{
            promotion.duration
          }}天，首页关键词数量{{ promotion.volume }}词
        </p>
      </div>
      <contract-ack type="content-rule" ref="contract" />
      <el-button class="submit" type="primary" @click="createPromotion">
        创建推广
      </el-button>
    </section>
  </div>
